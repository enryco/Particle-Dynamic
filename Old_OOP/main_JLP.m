...
boxSize = [10*350 5*350/2];   %[width hight] of the Box

axis equal;
axis([0 boxSize(1) -5*350/2 boxSize(2)]);
hold on
...
atoms(2).coordinates = [10*340.5 0];
atoms(1).direction = [1 0];
atoms(2).direction = [-1 0];
    ...
    i
    if atoms(i).coordinates(1) < round(boxSize(1)/2)
        atoms(i).color = 'red';
    else
        atoms(i).color = 'blue';
    end
