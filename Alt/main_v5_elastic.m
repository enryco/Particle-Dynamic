...
%% let the atoms fly
newVelocitiesX = zeros(numberOfAtoms);
newVelocitiesY = zeros(numberOfAtoms);
...
while 1
    ...
    for i=1:numberOfAtoms-1
        ...
        for j=i+1:numberOfAtoms
            ...
            if overlapCheck(coordinates(i,:),coordinates(j,:),radii(i),radii(j))
                ...
                [velAtom1,velAtom2] = impulseFunc(coordinates(i,:),coordinates(j,:),...
                    velocities(i,:),velocities(j,:));
                newVelocitiesX(i,j) = velAtom1(1);
                newVelocitiesY(i,j) = velAtom1(2);
                newVelocitiesX(j,i) = velAtom2(1);
                newVelocitiesY(j,i) = velAtom2(2);
    ...
    newVelocities = zeros(1,2);
    for i=1:numberOfAtoms
        newVelocities = [sum(newVelocitiesX(i,:)) sum(newVelocitiesY(i,:))];
        velocities(i,:) = velocities(i,:).*abs((newVelocities > 0)-1) + newVelocities;
    end
