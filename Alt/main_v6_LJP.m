...
boxSize = 1e6*[1 1];   %[width hight] of the Box
...
while 1
    ...
    velocitySum = sum(velocities);
   
    %Energy conservation
    velocityDiff = (velocitySumStart - velocitySum)/numberOfAtoms;
    velocities = velocities + ones(numberOfAtoms,1)*velocityDiff;
    
    
    %     velocitySumStart/sum(velocities)
    if (velocitySumStart/sum(velocities) == not(1)) ||...
            sum(sum(velocities > (radius/dt))) >= 1
        disp(velocities(find(sqrt(velocities(:,1).^2+velocities(:,2).^2) > (radius/dt) == 1,1),:));
        disp(velocitySumStart/sum(velocities));
